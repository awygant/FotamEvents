<!-- TODO: Make fixedUploadButton bring up the photo dialogue -->
<div class = "fixedUploadButton glassy">UPLOAD</div>
<div id = "menuButton" class = "glassy"></div>
<div id = "blinkingArrowContainer">
    <div class = "blinkingArrow"></div>
    <div class = "blinkingArrow"></div>
    <div class = "blinkingArrow"></div>
</div>
<div id = "menu">
    <div id = "menuLeftBar"></div>
    <div id = "menuInnerWrap">
        <a href = "index.php"><img class = "menuLogo" src = "img/logos/fotam-horiz.svg" onerror = "this.src='img/logos/fotam-horiz.png';this.onError=null;" alt = "Welcome to FOTAM"/></a>

        <ul>
            <li><a href = "javascript;">Gallery</a></li>
            <li><a href = "javascript;">Upload</a></li>
            <li><a href = "search.php">Find an Event</a></li>
            <li><a href = "logout.php">Logout</a></li>
            <!-- TODO: link to auth module settings -->
            <li><a href = "javascript;">Settings</a></li>
        </ul>
        <!-- TODO: Pull current user data & insert here -->
        <img class = "userProfilePic alignRight" src = "img/examples/imageTwo.jpg" alt = "Firstname Lastname" /><div class = "userInfoText"><p class ="username">Diddleydoodle</p></div>
    </div>
</div>